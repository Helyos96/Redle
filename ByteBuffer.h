...
class ByteBuffer
{
    public:
        ...
        constexpr static uint8_t InitialBitPos = 8;
        ...
        */
        ...
        void read(std::array<uint8_t, Size>& arr)
        {
            read(arr.data(), Size);
        }

        void ReadPackedUint64_t(uint64_t& guid)
        {
            guid = 0;
            ReadPackedUint64_t(read<uint8_t>(), guid);
        }

        void ReadPackedUint64_t(uint8_t mask, uint64_t& value)
        {
        ...
        std::string ReadString(uint32_t length, bool requireValidUtf8 = true);
        ...
        template <size_t Size>
        void append(std::array<uint8_t, Size> const& arr)
        {
        ...
        void AppendPackedUint64_t(uint64_t guid)
        {
            uint8_t mask = 0;
            size_t pos = wpos();
            *this << uint8_t(mask);
            ...
            uint8_t packed[8];
            if (size_t packedSize = PackUint64_t(guid, &mask, packed))
                append(packed, packedSize);

            put<uint8_t>(pos, mask);
        }

        static size_t PackUint64_t(uint64_t value, uint8_t* mask, uint8_t* result)
        {
            size_t resultSize = 0;
            *mask = 0;
            memset(result, 0, 8);
        ...
        void put(size_t pos, uint8_t const* src, size_t cnt);
